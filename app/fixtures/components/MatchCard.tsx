      ...
      className={`relative overflow-hidden rounded-2xl border transition-all duration-300 flex flex-col h-full ${
        timeStatus?.isLive
          ...
          : isToday
            ...
            : "bg-linear-to-b from-[#150e3d]/50 to-[#0c0824]/60 border-white/15 hover:border-white/25 hover:from-[#150e3d]/70 hover:to-[#0c0824]/80"
      }`}
    >
        ...
        <div className={`w-full px-4 py-2 sm:py-2.5 flex items-center justify-center gap-2 text-[11px] sm:text-xs font-extrabold tracking-wider ${
          timeStatus.isLive
            ...
            : timeStatus.isFuture
              ...
              : "bg-zinc-500/15 text-zinc-400 border-b border-white/15"
        }`}>
          {timeStatus.isLive && (
      ...
      <div className="flex flex-col gap-3.5 flex-grow p-4 sm:p-5">
        {/* Top Meta Row */}
        <div className="flex items-start justify-between gap-4 pb-2.5 border-b border-white/15">
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex flex-col gap-1 min-w-0">
              <div className="flex items-center gap-3">
                ...
                <span className={`text-sm sm:text-base font-bold ${hasPlayed && !isWinner(match, 1) ? "text-zinc-400" : "text-white"} truncate`}>
                  {match.team1}
                </span>
              </div>
              {/* Scorers for Team 1 */}
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className={`w-9 h-9 rounded-lg flex items-center justify-center border font-black text-sm sm:text-base transition-all duration-300 ${
                hasPlayed
                  ? (isWinner(match, 1) ? "border-emerald-500/35 bg-emerald-500/10 text-emerald-400" : "border-white/15 bg-white/[0.02] text-zinc-400")
                  : "border-white/15 bg-white/[0.02] text-zinc-400"
              }`}>
                {score1}
              </span>
            </div>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex flex-col gap-1 min-w-0">
              <div className="flex items-center gap-3">
                ...
                <span className={`text-sm sm:text-base font-bold ${hasPlayed && !isWinner(match, 2) ? "text-zinc-400" : "text-white"} truncate`}>
                  {match.team2}
                </span>
              </div>
              {/* Scorers for Team 2 */}
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className={`w-9 h-9 rounded-lg flex items-center justify-center border font-black text-sm sm:text-base transition-all duration-300 ${
                hasPlayed
                  ? (isWinner(match, 2) ? "border-emerald-500/35 bg-emerald-500/10 text-emerald-400" : "border-white/15 bg-white/[0.02] text-zinc-400")
                  : "border-white/15 bg-white/[0.02] text-zinc-400"
              }`}>
                {score2}
              </span>
